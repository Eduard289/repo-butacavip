...
def mainlist(item):
    ...
    # (ELIMINADO: Cuestiones Preliminares)
    # (ELIMINADO: Personalizar Búsquedas)
    ...
    if not config.get_setting('mnu_simple', default=False):
        ...
        # (ELIMINADO: Torrents, Doramas, Animes, Adultos, Youtube)
...
def show_infos(item):
    logger.info()
    itemlist = []
    # (Esta función queda residual pero no molesta, ya no se llama desde el menú)
    return itemlist
...
def do_search(item, tecleado):
    ...
    if item.search_type == 'movie': channels_search_excluded = config.get_setting('search_excludes_movies', default='')
    elif item.search_type == 'tvshow': channels_search_excluded = config.get_setting('search_excludes_tvshows', default='')
    elif item.search_type == 'documentary': channels_search_excluded = config.get_setting('search_excludes_documentaries', default='')
    elif item.extra == 'only_torrents': channels_search_excluded = config.get_setting('search_excludes_torrents', default='')
    else:
    ...
    # ~ Buscar desde Filmaffinity/Tmdb
    if item.from_channel != '':
        desde_lists = True
